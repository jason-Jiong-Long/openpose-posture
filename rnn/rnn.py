...
class RNN_Model(nn.Module):
    def __init__(self):
        ...
        self.rnn = nn.RNN(input_size=self.input_size, hidden_size=self.hidden_size,
                          num_layers=self.num_layers, batch_first=True) #RNN
    ...
    def forward(self,x):
        ...
        out, hn = self.rnn(x, h_0) #RNN with input, hidden, and internal state
...
def fit_model(RNN_model, loss_func, RNN_optimizer, num_epochs, train_loader, test_loader, input_shape):
    # Traning the Model
...
if __name__ == "__main__":
    ...
    #RNN_model = RNN_Model(input_size, hidden_size, num_classes)
    RNN_model = RNN_Model()
    #RNN_model = RNN_model.float()
    LR = 0.001
    loss_func = nn.CrossEntropyLoss()   # the target label is not one-hotted
    print(RNN_model)
    RNN_optimizer = torch.optim.Adam(RNN_model.parameters(), lr=LR)   # optimize all cnn parameters

    input_shape = (-1,4,36)


    training_loss, training_accuracy, validation_loss, validation_accuracy = fit_model(RNN_model, loss_func, RNN_optimizer, num_epochs, train_loader, test_loader, input_shape)
